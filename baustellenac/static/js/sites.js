...
$.fn.sites = function(opts) {
  var create_infomodal, icon_default, icon_sidewalk, init, make_infopopup, make_marker, make_route, map, map_attribution, map_url, map_zoom, markers, max_zoom, onLocationFound, show_infomodal;
  if (opts == null) opts = {};
  map_zoom = 13;
  max_zoom = 18;
  ...
  make_marker = function(elem) {
    ...
    if (start_lat !== 'None' && start_lng !== 'None') {
      ...
      icon = icon_default;
      if (elem.data('sidewalk_only') === 'True') icon = icon_sidewalk;
  ...
  make_infopopup = function(elem) {
    ...
    if (elem.data('sidewalk_only') === 'True') info += ' (Nur auf dem Gehweg)';
    info += '<br/>';
